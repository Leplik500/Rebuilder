...
public class UserService_UpdateUserProfileAsync_Tests
{
    ...
    public UserService_UpdateUserProfileAsync_Tests()
    {
        ...
        repoMock.Verify(
            r =>
                r.GetFirstOrDefaultAsync(
                    It.IsAny<Expression<Func<UserProfile, bool>>>(),
                    It.IsAny<
                        Func<IQueryable<UserProfile>, IOrderedQueryable<UserProfile>>
                    >(),
                    It.IsAny<
                        Func<
    ...
    public async Task UpdateUserProfileAsync_ReturnsNotFound_WhenUserProfileDoesNotExist()
    {
        ...
        // Настраиваем репозиторий так, чтобы он не нашел профиль (возвращает null)
        repoMock
            .Setup(r =>
                r.GetFirstOrDefaultAsync(
                    It.IsAny<Expression<Func<UserProfile, bool>>>(),
        ...
        // Проверяем, что GetFirstOrDefaultAsync был вызван для поиска профиля
        repoMock.Verify(
            r =>
                r.GetFirstOrDefaultAsync(
                    It.Is<Expression<Func<UserProfile, bool>>>(expr => true), // профиль ищется по UserId
                    null,
                    null,
                    false,
                    false,
        ...
        // Проверяем, что Update НЕ был вызван, так как профиль не найден
        repoMock.Verify(r => r.Update(It.IsAny<UserProfile>()), Times.Never);
    }

    [Fact]
    public async Task UpdateUserProfileAsync_UpdatesAllFields_WhenAllFieldsProvided()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        capturedProfile.Age.Should().Be(updateDto.Age!.Value);
        capturedProfile.Gender.Should().Be(Gender.Female);
        capturedProfile.ActivityLevel.Should().Be(ActivityLevel.High);
        capturedProfile.FitnessGoal.Should().Be(FitnessGoal.FormMaintence);
    ...
    public async Task UpdateUserProfileAsync_UpdatesOnlyProvidedFields_WhenPartialUpdateRequested()
    {
        ...
        capturedProfile.ActivityLevel.Should().Be(ActivityLevel.Low);
    ...
    [Fact]
    public async Task UpdateUserProfileAsync_ReturnsValidationError_WhenInvalidWeightProvided()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        var invalidUpdateDto = new UpdateUserProfileDto(
            Weight: -10, // Невалидный вес (отрицательное значение)
            Height: null,
            Age: null,
            Gender: null,
            ActivityLevel: null,
        ...
        // Проверяем, что методы репозитория не вызывались из-за ошибки валидации
        repoMock.Verify(
            r =>
                r.GetFirstOrDefaultAsync(
                    It.IsAny<Expression<Func<UserProfile, bool>>>(),
    ...
    [Fact]
    public async Task UpdateUserProfileAsync_ReturnsValidationError_WhenInvalidHeightProvided()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        var invalidUpdateDto = new UpdateUserProfileDto(
            Weight: null,
            Height: -5, // Невалидный рост (отрицательное значение)
            Age: null,
            Gender: null,
            ActivityLevel: null,
            FitnessGoal: null
    ...
    [Fact]
    public async Task UpdateUserProfileAsync_ReturnsValidationError_WhenInvalidAgeProvided()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        var invalidUpdateDto = new UpdateUserProfileDto(
            Weight: null,
            Height: null,
            Age: -1, // Невалидный возраст (отрицательное значение)
            Gender: null,
            ActivityLevel: null,
            FitnessGoal: null
        );
    ...
    [Fact]
    public async Task UpdateUserProfileAsync_UpdatesModifiedDate_WhenProfileUpdated()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        capturedProfile.FitnessGoal.Should().Be(FitnessGoal.WeightLoss);
        capturedProfile.UpdatedAt.Should().HaveValue();
        capturedProfile.UpdatedAt.Should().BeAfter(initialUpdatedAt);
    ...
    public async Task UpdateUserProfileAsync_HandlesCancellationTokenProperly()
    {
        ...
        // Создаем CancellationTokenSource для отмены операции
        var cts = new CancellationTokenSource();
        await cts.CancelAsync(); // Отменяем токен сразу
        ...
        repoMock
            ...
            .ThrowsAsync(
                new OperationCanceledException("Operation was cancelled", cts.Token)
            );
        ...
        // Проверяем, что GetFirstOrDefaultAsync был вызван с правильным CancellationToken
        repoMock.Verify(
            r =>
                r.GetFirstOrDefaultAsync(
                    It.IsAny<Expression<Func<UserProfile, bool>>>(),
        ...
        // Проверяем, что Update не был вызван из-за отмены операции
        repoMock.Verify(r => r.Update(It.IsAny<UserProfile>()), Times.Never);
    }
    ...
    public async Task UpdateUserProfileAsync_HandlesRepositoryException_WhenDatabaseErrorOccurs()
    {
        ...
        // Имитируем ошибку базы данных при вызове Update
        repoMock
            .Setup(r => r.Update(It.IsAny<UserProfile>()))
        ...
        // Проверяем, что GetFirstOrDefaultAsync был вызван
        repoMock.Verify(
            r =>
                r.GetFirstOrDefaultAsync(
                    It.IsAny<Expression<Func<UserProfile, bool>>>(),
        ...
        // Проверяем, что Update был вызван, но завершился с исключением
        repoMock.Verify(r => r.Update(It.IsAny<UserProfile>()), Times.Once);
    }
    ...
    [Fact]
    public async Task UpdateUserProfileAsync_PreservesUnchangedFields_WhenPartialUpdatePerformed()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        capturedProfile.Should().NotBeNull();
        capturedProfile!.UserId.Should().Be(userId);
        capturedProfile.Weight.Should().Be(existingProfile.Weight);
        capturedProfile.Height.Should().Be(partialUpdateDto.Height!.Value);
        capturedProfile.Age.Should().Be(existingProfile.Age);
        capturedProfile.Gender.Should().Be(Gender.Male);
        capturedProfile.ActivityLevel.Should().Be(ActivityLevel.Average);
        capturedProfile.FitnessGoal.Should().Be(FitnessGoal.WeightLoss);
    ...
    [Fact]
    public async Task UpdateUserProfileAsync_ValidatesEnumValues_WhenInvalidEnumProvided()
    {
        // Arrange
        userContextMock.Setup(c => c.IsAuthenticated).Returns(true);
        userContextMock.Setup(c => c.UserId).Returns(userId);
        ...
        var invalidUpdateDto = new UpdateUserProfileDto(
            ...
            ActivityLevel: "InvalidActivityLevel", // Невалидное значение
            FitnessGoal: "InvalidFitnessGoal" // Невалидное значение
        );
